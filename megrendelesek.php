<?php
	...
	if ($_GET['action'] == 'edit') {
		...
		FROM
			megrendelesek
		WHERE
			idMegrendelesNr = ' . $_GET['id'];
			
		$talalat = $adatbazis->query($lekerdezes);
		$sor = $talalat->fetch_assoc();
		$idMegrendelesNr = $sor['idMegrendelesNr'];
		$idMegrendeles = $sor['idMegrendeles'];
		...
		$idMegrendelesNr = $sor['idMegrendelesNr'] + 1;
		$idMegrendeles = '';
		$idProjektNr = '';
		$eutvonal = '';
		$efajlnev = '';
...
<html>
	...
	<body>
		<form action="commit.php?action=<?php echo $_GET['action']; ?>&type=megrendelesek" method="post">
			<table>
				<tr class="sorok" >
					<td> Ajánlat sorszáma:</td>
					<td> <?php echo $idMegrendelesNr; ?></td>
				</tr>
				
				<tr class="sorok" >
					<td> Azonosító:</td>
					<td> <input type="text" name="idMegrendeles" value="<?php echo $idMegrendeles; ?>"/> </td>
				</tr>
				...
				<tr class="sorok" >
					<td colspan="2" style="text">
						<?php
							if ($_GET['action'] == 'edit') {
								echo '<input type="hidden" value="' . $_GET['id'] . '" name="idMegrendelesNr" /> ';
							}
						?>
